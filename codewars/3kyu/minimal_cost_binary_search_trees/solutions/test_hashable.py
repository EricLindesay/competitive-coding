...
def display_tree(tree: Tree, n: float, depth=1) -> None:
    if not tree:
        return ""
    '''
    n = int(n)
    ...
    print([(str(x[0]),x[1]) for x in bfs(tree)])
    
    order = bfs(tree)
    # i basically need to do a BFS to get each one on the line, then i can do a \n after that.
    
    depth = 1
    i = 0
    while i < len(order):
        for j in range(len(order)):
            if order[j][1] == depth:  # if its on the same depth


        string = f"{spaces*n}{str(order[i][0])}"
        string += "\n"
        depth += 1
    '''
    if not tree:
    ...
    while queue:
        val = queue.pop(0)
        # instead of appending, add it to string
        # if depth != val[1], print a newline first
        ans.append((val[0].root, val[1]))
        if val[0].left:
            queue.append((val[0].left,val[1]+1))
        if val[0].right:
            queue.append((val[0].right,val[1]+1))

    return ans    
    
    #string += f"{display_tree(tree.left, n/2)} {spaces*n}{display_tree(tree.right, n/2)}"
    
    #string += f"{spaces*int(n/2)}{tree.left.root} {spaces*n}{tree.right.root}"
    return string


def bfs(tree: Tree) -> list:
    if not tree:
        return []
...
if __name__ == "__main__":
    ...
    all_nodes = [node3, node4, node6, node7, node8, node9]

    top_three = sorted(zip(range(len(all_nodes)), all_nodes), reverse=True, key=lambda x: x[1].weight)
